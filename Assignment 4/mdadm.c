  ...
  while (selected_addr < (addr + len)) {
    ...
    if(cache_enabled()){
      ...
      if(findCache == -1){
        ...
        jbod_operation(encode_op(JBOD_READ_BLOCK, 0, 0), local_buf);
...
int mdadm_write(uint32_t addr, uint32_t len, const uint8_t *buf) {
  ...
  while (selected_addr < (addr + len)){
    ...
    int findCache = cache_lookup(disk_num, block_num, buf);
      if(findCache == 1){
        jbod_operation(encode_op(JBOD_WRITE_BLOCK, 0, 0), local_buf);
    ...
    disk_test = jbod_operation(encode_op(JBOD_SEEK_TO_DISK, disk_num, 0), NULL);
    block_test = jbod_operation(encode_op(JBOD_SEEK_TO_BLOCK, 0, block_num), NULL);
    if ((disk_test != 0) && (block_test != 0)) {
      return -1;
    }         
    ...
    if (jbod_operation(encode_op(JBOD_WRITE_BLOCK, 0, 0), local_buf)== -1){
      if(cache_enabled()){
