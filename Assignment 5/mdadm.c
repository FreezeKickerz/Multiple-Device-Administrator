...
int mdadm_read(uint32_t addr, uint32_t len, uint8_t *buf) {
  ...
  while (selected_addr < (addr + len)) {
    ...
    if(cache_enabled()){
      int findCache = cache_lookup(disk_num, block_num, local_buf);
      if(findCache == -1){
        ...
        jbod_client_operation(encode_op(JBOD_READ_BLOCK, 0, 0), local_buf);
        cache_insert(disk_num, block_num, local_buf);
      }
    }
    else{
      int disk_test = jbod_client_operation(encode_op(JBOD_SEEK_TO_DISK, disk_num, 0), NULL);
      int block_test = jbod_client_operation(encode_op(JBOD_SEEK_TO_BLOCK, 0, block_num), NULL);
      if ((disk_test != 0) && (block_test != 0)) {
        return -1;
      }          
      
      if (jbod_client_operation(encode_op(JBOD_READ_BLOCK, 0, 0), local_buf) == -1) {
        return -1;
      }
    }
    if (addr_offset == 0) {
...
int mdadm_write(uint32_t addr, uint32_t len, const uint8_t *buf) {
  ...
  while (selected_addr < (addr + len)){
    ...
    int findCache = cache_lookup(disk_num, block_num, buf);
      if(findCache == 1){
        jbod_client_operation(encode_op(JBOD_WRITE_BLOCK, 0, 0), local_buf);
    ...
    disk_test = jbod_client_operation(encode_op(JBOD_SEEK_TO_DISK, disk_num, 0), NULL);
    block_test = jbod_client_operation(encode_op(JBOD_SEEK_TO_BLOCK, 0, block_num), NULL);
    if ((disk_test != 0) && (block_test != 0)) {
      return -1;
    }         

    if(addr_offset == 0){
      if((len - bytes_wrote) < JBOD_BLOCK_SIZE){
        memcpy(local_buf, &buf[bytes_wrote], (len - bytes_wrote));
        selected_addr += (len - bytes_wrote);
        bytes_wrote += (len - bytes_wrote);
      }
      else{
        memcpy(local_buf, &buf[bytes_wrote], JBOD_BLOCK_SIZE);
        selected_addr += JBOD_BLOCK_SIZE;
        bytes_wrote += JBOD_BLOCK_SIZE;
      }      
    }
    if(addr_offset != 0){
      if((len + addr_offset) < JBOD_BLOCK_SIZE){
        memcpy((local_buf + addr_offset), buf, len);
        selected_addr += len;        
      }
      else{
        memcpy((local_buf + addr_offset), buf,(JBOD_BLOCK_SIZE - addr_offset));
        selected_addr += (JBOD_BLOCK_SIZE - addr_offset);
        bytes_wrote += (JBOD_BLOCK_SIZE - addr_offset);        
      }
    }
    if (jbod_client_operation(encode_op(JBOD_WRITE_BLOCK, 0, 0), local_buf)== -1){
      if(cache_enabled()){
        cache_update(disk_num,block_num,local_buf);
      }
	    return -1;
    }    
  }
